...
MODEL_DICT = {"ViT-L-14": "/share/sail/hexinyi/LanguageBind-new/pretrained_model/CLIP-ViT-L-14-DataComp",
              "ViT-H-14": "laion/CLIP-ViT-H-14-laion2B-s32B-b79K"}
CHECKPOINT_DICT = {"ViT-L-14": "/share/sail/hexinyi/LanguageBind-new/pretrained_model/CLIP-ViT-L-14-DataComp/pytorch_model.bin",
...
def main(args):
    ...
    # 增添测试集
    import copy
    if "SUNRGBD" not in args.val_d_cls_data:
        args_copy = copy.deepcopy(args)  # 创建args的一个深拷贝副本
        args_copy.val_d_cls_data = ["SUNRGBD"]
        args_copy.do_train = False
        data_sun = get_data(args_copy, epoch=start_epoch)
    if "NYUV2" not in args.val_d_cls_data:
        args_copy = copy.deepcopy(args)  # 创建args的一个深拷贝副本
        args_copy.val_d_cls_data = ["NYUV2"]
        args_copy.do_train = False
        data_nyu = get_data(args_copy, epoch=start_epoch)   
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        clip_type = ('image',)
        modelrgb = LanguageBind(clip_type=clip_type, cache_dir='/share/sail/hexinyi/LanguageBind-new/pretrained_model')
        modelrgb = modelrgb.to(device)
        modelrgb.eval()


        train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, modelrgb, tb_writer=writer)
