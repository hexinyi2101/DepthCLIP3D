...
class LanguageBind(nn.Module):
    def __init__(self, clip_type=('thermal', 'image', 'video', 'depth', 'audio'), use_temp=True, cache_dir='./cache_dir'):
        ...
        for c in clip_type:
            pretrained_ckpt = f'/share/sail/hexinyi/LanguageBind-new/pretrained_model/LanguageBind/LanguageBind_{c.capitalize()}'
    ...
    def forward(self, inputs):
        ...
        for key, value in inputs.items():
            features = self.modality_encoder[key](**value)[0]  # Assuming the model's first output is what you want
            patch_features[key] = features  # Store patch-level features

            value = self.modality_encoder[key](**value)[1]
            before_proj[key] = value #new
