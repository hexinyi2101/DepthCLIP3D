...
def run(model, classifier, dataloader, args):
    ...
    with torch.no_grad():
        ...
        for images, target in tqdm(dataloader, unit_scale=args.batch_size):
            ...
            with autocast():
                # predict
                output = model(image=images, myeval=True)
    ...
    '''
    num_classes = len(CLASSNAMES[args.val_d_cls_data])
    # 初始化收集每个类别的预测和真实标签的存储
    predictions = {cls: [] for cls in range(num_classes)}
    labels = {cls: [] for cls in range(num_classes)}
    ...
    with torch.no_grad():
        for images, target in tqdm(dataloader, unit_scale=args.batch_size):
            ...
            for i in range(num_classes):
                class_mask = target == i
                predictions[i].extend(preds[class_mask].tolist())
                labels[i].extend(target[class_mask].tolist())

    # 计算每个类别的准确率
    class_accuracies = {}
    for cls in range(num_classes):
        correct = sum(1 for pred, label in zip(predictions[cls], labels[cls]) if pred == label)
        total = len(labels[cls])
        class_accuracies[CLASSNAMES[args.val_d_cls_data][cls]] = correct / total if total > 0 else 0
